...
@Slf4j
@Component
public class OrderRefundProducer extends AbstractCommonSendProduceTemplate<OrderRefundMqDTO> {

    @Override
    protected BaseSendExtendDTO buildBaseSendExtendParam(OrderRefundMqDTO messageSendEvent) {
        return BaseSendExtendDTO.builder()
                .eventName("修改订单状态为退款")
                .keys(String.valueOf(messageSendEvent.getOrderSn()))
                .topic(RocketMqConstant.ORDER_TOPIC)
                .tag(RocketMqConstant.ORDER_REFUND_TAG)
                .sentTimeout(2000L)
                .build();
    }

    @Override
    protected Message<?> buildMessage(OrderRefundMqDTO messageSendEvent, BaseSendExtendDTO requestParam) {
